	...
	if (m_pViewer != nullptr)
	{
		if (ea.getEventType() == ea.FRAME)
		{
			...
			if (m_pViewer->computeIntersections(eaX, eaY, results))
			{
				...
				osgEarth::Util::Viewpoint vp = OECore::ins()->getManipulator()->getViewpoint();
				QString ViewPInfo = "";
...
void ViewpointHandler::createInfoLabel()
{
	...
	std::string strFont = FileUtils::ins()->getPath(std::string("msyh.ttf"));
	font = osgText::readFontFile(strFont);
	if (font == NULL)
	{
		std::cout<<"release下没有读到字体."<<std::endl;
	}
	if (canvas != NULL )
	{
		VBox* grid = new VBox;
