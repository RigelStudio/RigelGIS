...
osg::Vec2Array* MEMath::calcStripTexCoord(osg::Vec3dArray* source)
{
...
osg::Vec2Array* MEMath::calcPipeTexCoord(osg::Vec3dArray* source, size_t parts)
{
	osg::Vec2Array* texCoords = new osg::Vec2Array;
	auto count = source->size();
	if (count < 2)
...
osg::Vec3dArray* MEMath::createPipe(osg::Vec3dArray* source, float radius)
{
		...
		count = source->size();
 		for (int i = 0; i < count; i++)
 		{
 			...
 			for (size_t j = 0; j < parts; ++j, angle += angleDelta)
 			{
 				float c = cosf(angle);
 				float s = sinf(angle);
 				vertex->push_back(source->at(j) + osg::Vec3d(c*radius, s*radius, 0.0f));
 			}
 		}
	}
	return vertex;
...
osg::Vec3dArray* MEMath::BezierCurve(osg::Vec3dArray* vertexs, float perc, size_t parts)
{
	auto controlPoints = new osg::Vec3dArray;
	auto resultCurve = new osg::Vec3dArray;
